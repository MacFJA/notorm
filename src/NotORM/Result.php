 ...
 */
class Result extends AbstractBase implements \Iterator, \ArrayAccess, \Countable, JsonSerializable
{
    protected $single;
        ...
        foreach ($matches[1] as $names) {
            ...
            if ($names != $parent . '.') { // case-sensitive
                ...
                foreach ($matches as $match) {
                    ...
                    $column        = ($delimiter == ':'? $this->notORM->structure->getPrimary(
                        $parent
                    ) : $this->notORM->structure->getReferencedColumn($name, $parent));
                    $primary       = ($delimiter == ':'? $this->notORM->structure->getReferencedColumn(
                        $parent,
                        $table
    ...
    function __toString()
    {
        ...
        if ($this->union) {
            $return = ($this->notORM->driver == 'sqlite' || $this->notORM->driver == 'oci'? $return : '(' . $return . ')') . implode(
                    $this->union
                );
            if ($this->unionOrder) {
    ...
    protected function query($query, $parameters)
    {
        if ($this->notORM->debug) {
            if (!is_callable($this->notORM->debug)) {
                ...
                $backtrace = array('file' => '', 'line' => 0); //Avoid warning about 'might have not been defined'
                foreach (debug_backtrace() as $backtrace) {
                    if (isset($backtrace['file']) && !preg_match(
                            $pattern,
                            $backtrace['file']
                        )
                    ) { // stop on first file outside NotORM source codes
                        break;
                    }
                }
                error_log($backtrace['file'] . ':' . $backtrace['line'] . ':' . $debug . PHP_EOL, 0);
    ...
    function insert($data)
    {
        ...
        if (!isset($data[$this->primary]) && ($id = $this->notORM->connection->lastInsertId(
                $this->notORM->structure->getSequence($this->table)
            ))
        ) {
    ...
    function update(array $data)
    {
        ...
        $return = $this->query(
            'UPDATE' . $this->topString($this->limit, $this->offset) . ' ' . $this->table . ' SET ' . implode(
                ', ',
                $values
            ) . $this->whereString(),
            $parameters
     ...
     */
    function insertUpdate(array $unique, array $insert, array $update = array())
    {
        if (!$update) {
            $update = $insert;
        }
        ...
        if ($this->notORM->driver == 'mysql') {
            ...
            try {
                ...
                ) { // '23000' - duplicate key, '23505' unique constraint pgsql
                    if (!$update) {
                        return 0;
                    }
                    $clone  = clone $this;
                ...
                if ($errorMode == PDO::ERRMODE_EXCEPTION) {
                    ...
                    trigger_error(
                        'PDOStatement::execute(): ' . $e->getMessage(),
                        E_USER_WARNING
                    ); // E_WARNING is unusable
                }
            }
        }
    ...
    function delete()
    {
        ...
        $return = $this->query(
            'DELETE' . $this->topString($this->limit, $this->offset) . ' FROM ' . $this->table . $this->whereString(),
            $this->parameters
        );
        if (!$return) {
            return false;
     ...
     *
     * @param string $condition  condition possibly containing ? or :name
     * @param array  $parameters array accepted by PDOStatement::execute or a scalar value
     * @param        mixed       ...
     * @return Result fluent interface
     */
     ...
     *
     * @return null
     * @throws \Exception|null|\PDOException
     */
    protected function execute()
    {
        if (!isset($this->rows)) {
            ...
            foreach (array_merge(
                         $this->select,
                         array($this, $this->group, $this->having),
                         $this->order,
                         $this->unionOrder
                     ) as $val) {
    ...
    function fetchPairs($key, $value = '')
    {
        ...
        foreach ($clone as $row) {
            ...
            $return[(string)$values[0]] = ($value != ''? $values[(array_key_exists(
                1,
                $values
            )? 1 : 0)] : $row); // isset($values[1]) - fetchPairs('id', 'id')
        }
        return $return;
    }
